...
CREATE  TABLE IF NOT EXISTS `mydb`.`SeatInstanceTemplate` (
  `idAirline` VARCHAR(45) NOT NULL ,
  `idTemplate` DECIMAL(4,0) NOT NULL ,
  `airplaneType` INT NOT NULL ,
  `seatNumber` INT NOT NULL ,
  `price` DOUBLE NOT NULL ,
  PRIMARY KEY (`idTemplate`, `idAirline`, `airplaneType`, `seatNumber`) ,
  ...
  CONSTRAINT `seatinstancetemplate_template`
    FOREIGN KEY (`idTemplate` , `idAirline` )
    REFERENCES `mydb`.`Template` (`idAirplaneType` , `idAirline` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE INDEX `seatinstancetemplate_template_idx` ON `mydb`.`SeatInstanceTemplate` (`idTemplate` ASC, `idAirline` ASC) ;
