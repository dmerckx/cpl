...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`ActualFlight` (`idFlight` INT, `idAirplaneType` INT, `idPeriod` INT, `idAirpline` INT, `idTemplate` INT, `fromDate` INT, `departure` INT, `arrival` INT);
	...
	periodLoop : LOOP
		...
		CREATE TEMPORARY TABLE dates_to_insert AS (
			SELECT
				...
				FROM time_intervals as datesTable JOIN period as p ON p.idPeriod='1'
					WHERE NOT EXISTS (
						SELECT * FROM periodgenerated as pg WHERE pg.generatedDate = datesTable.interval_from
		...
		INSERT INTO flight(`idAirline`, `idTemplate`)
			SELECT my_idAirline, my_idTemplate FROM `dates_to_insert`;
