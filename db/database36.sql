...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`ActualSeatInstances` (`idFlight` INT, `idTemplate` INT, `idAirplaneType` INT, `seatNumber` INT, `idSeatType` INT, `seatTypeName` INT, `price` INT);
...
USE `mydb`;
DROP procedure IF EXISTS `mydb`.`update_seat_instance_templates`;

DELIMITER $$
USE `mydb`$$
CREATE PROCEDURE update_seat_instance_templates(
	IN input_idAirline INT,
	IN input_idTemplate INT)
BEGIN
	-- Variable declarations
